...
//
// Nachos's condition variables should work according to the “Mesa” style.
...
public:
    Channel(const char *debugName, int buffSize);
    ~Channel();

    const char *GetName() const;

    // Send atomically waits to some thread to call Receive.

...
main(int argc, char **argv)
{
    ...
    for (argc--, argv++; argc > 0; argc -= argCount, argv += argCount) {
        ...
        if (!strcmp(*argv, "-x")) {          // Run a user program.
            ...
            DEBUG('t', "Halt called from main (1) \n");  
            ...
            if(argc == 1){
                ...
                DEBUG('t', "SynchConsoleTest doesn't take arguments.\n");
            }
            DEBUG('t', "Halt called from main (2) \n");
            interrupt->Halt();
        } 
#endif
#ifdef FILESYS
        if (!strcmp(*argv, "-cp")) {         // Copy from UNIX to Nachos.
            ...
            if(fileSystem->Remove(*(argv + 1))){
              printf("Removed file %s succesfully.\n", *(argv + 1));
            } else {
              printf("Failed removing file %s.\n", *(argv + 1));
            }
            argCount = 2;
        } else if (!strcmp(*argv, "-ls")) {
            fileSystem->List(argv[1]);       // List Nachos directory.
            printf("\n");
            ...
            Path *path = new Path(*(argv + 1));
            
            if(fileSystem->ChangeDirectory(path)){
              printf("Changed to directory %s succesfully.\n", *(argv + 1));
            } else {
              printf("Failed changing to directory %s.\n", *(argv + 1));
            }
            argCount = 2;
        } else if (!strcmp(*argv, "-mkdir")){
            ASSERT(argc > 1);
            if(fileSystem->Create(*(argv + 1), 0, true)){
              printf("Created directory %s succesfully.\n", *(argv + 1));
            } else {

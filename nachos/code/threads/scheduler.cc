...
Scheduler::ReadyToRun(Thread *thread)
{
    ...
    readyList->SortedInsert(thread,thread->GetPriority(), false);
...
void
Scheduler::ChangePriority(int priority, Thread *thread){
    DEBUG('a',"Changing priority of thread %s from %d to %d \n",thread->GetName(), thread->GetPriority(), priority);
    if (readyList->Has(thread)){
    readyList->Remove(thread);
    readyList->SortedInsert(thread, priority, false);
...
Scheduler::GetMaxPriority(){
    ...
    if(!readyList->IsEmpty())
        return readyList->Head()->GetPriority();
    else
        return 0;
}
#endif

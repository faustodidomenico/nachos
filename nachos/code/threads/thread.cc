...
Thread::Thread(const char *threadName, bool isJoinable_,int priority_, unsigned dirSector_) 
{
    name       = threadName;
    stackTop   = nullptr;
    stack      = nullptr;
    status     = JUST_CREATED;
    isJoinable = isJoinable_;
    channelJoin = nullptr;
    priority = priority_;
    
    if(isJoinable)
        channelJoin = new Channel("Join Channel", 10);
    ...
    fileTable->Add(nullptr);
    processId = processTable->Add(this);
#endif
...
Thread::~Thread()
{
    ...
    DEBUG('t', "Deallocating thread stack \"%s\"\n", name);
    if (stack != nullptr)
    ...
    DEBUG('t', "Removing thread from process table \"%s\"\n", name);
...
/// Block al llamante hasta que el hilo en
/// cuestión termine.
...
Thread::AddFile(OpenFile *file){
    ASSERT(file != nullptr);
    int openFileId = fileTable->Add(file);
    ASSERT(openFileId > 0);
    return openFileId;
}
...
bool
Thread::HasOpenFile(OpenFileId id){
    return fileTable->HasKey(id);
}
#endif
...
void 
Thread::SetDirSector(unsigned newSector){
    dirSector = newSector;
}
#endif
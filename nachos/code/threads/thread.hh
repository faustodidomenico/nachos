...
public:
    ...
    /// Initialize a `Thread`.
    Thread(const char *debugName, bool isJoinable_ = false ,int priority_ = 0, unsigned dirSector_ = 1);
    ...
    bool IsJoinable();
    #ifdef DIRECTORY
    unsigned GetDirSector();
    void SetDirSector(unsigned newSector);
    #endif
...
public:
    ...
    OpenFileId AddFile(OpenFile *archivo);
	
	void RemoveFile(OpenFileId id);

	OpenFile *GetFile(OpenFileId id);

    bool HasOpenFile(OpenFileId id);

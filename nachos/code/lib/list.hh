...
public:
    ...
    /// Put item into list.
    void SortedInsert(Item item, int sortKey, bool ascendent = true);
...
template <class Item>
void
List<Item>::SortedInsert(Item item, int sortKey, bool ascendent)
{
    if(ascendent){
...
template <class Item>
Item
List<Item>::FindByKey(int keyPtr)
{
  for (ListNode *ptr = first; ptr != nullptr; ptr = ptr->next) {
      if (keyPtr == ptr->key) 
        return ptr->item;      

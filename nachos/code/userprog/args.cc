...
bool CountArgsToSave(int address, unsigned *count)
{
    ...
    unsigned c = 0;
    do {
        ASSERT(SafeReadMem(address + 4 * c, 4, &val));
        c++;
...
SaveArgs(int address)
{
    ...
    for (unsigned i = 0; i < count; i++) {
        ...
        ASSERT(SafeReadMem(address + i * 4, 4, &strAddr));
...
WriteArgs(char **args)
{
    ...
    for (unsigned i = 0; i < c; i++)
        ASSERT(SafeWriteMem(sp + 4 * i, 4, argsAddress[i]));
    ASSERT(SafeWriteMem(sp + 4 * c, 4, 0));  // The last is null.
